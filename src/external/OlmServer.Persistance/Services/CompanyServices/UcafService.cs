...
namespace OlmServer.Persistance.Services.CompanyServices
{
    public class UcafService : IUcafService
    {
        ...
        public async Task<UniformChartOfAccount> GetByCodeAsync(string companyId, string code, CancellationToken cancellationToken)
        {
            ...
            return await _queryRepository.GetByExpressionAsync(p => p.Code == code, cancellationToken);
        }
        ...
        public async Task CreateMainUcafsToCompanyAsync(string companyId, CancellationToken cancellationToken)
        {
            ...
            var oldList = await _queryRepository.GetAllWhere(p => p.Type == "A").ToListAsync();
            _commandRepository.DeleteRange(oldList);
        ...
        public async Task<bool> CheckRemoveByIdUcafIsGroupAndAvailable(string id, string companyId)
        {
            ...
            UniformChartOfAccount ucaf = await _queryRepository.GetByIdAsync(id, false);
            if (ucaf.Type == "G")
            {
                IList<UniformChartOfAccount> list = await _queryRepository.GetAllWhere(p => p.Code.StartsWith(ucaf.Code) && p.Type == "M").ToListAsync();
                if (list.Count() > 0)
                {
                    return false;
                }
        ...
        public Task UpdateUcafAsync(UniformChartOfAccount uniformChartOfAccount, string companyId)
        {
            _context = (CompanyDbContext)_contextService.CreateDbContextInstance(companyId);
            _commandRepository.SetDbContextInstance(_context);
            _unitOfWork.SetDbContextInstance(_context);

            _commandRepository.Update(uniformChartOfAccount);
